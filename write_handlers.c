		...
		int length, char padd, char extra_c)
{
	...
	if (width > length)
	{
		...
		else if (!(flags & F_MINUS) && padd == ' ')
		{
			if (extra_c)
				buffer[--ind] = extra_c;
			return (write(1, &buffer[padd_start], i - padd_star) +
		...
		int width, int precision, int size)
{
	...
	if (width > length)
	{
		...
		{
			return (write(1, &buffer[0], i) + write
					(1, &buffer[ind], length));
		}
	}
	return (write(1, &buffer[ind], length));
 ...
 * @flags: flags specifier
 * @padd: char representing padd
 * @extra_c: char5 representing
 * @padd_start: index at which padding should start
 * Return: index at which padding should start
 */
